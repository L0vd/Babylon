...
echo ''
echo -e 'INSTALLING BABYLON NODE MONITORING'
...
# make the telegraf user sudo and adm to be able to execute scripts as babylon user
...
#Babylon monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/babylond or /home/user/go/bin/babylond
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: bbnvaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
[[outputs.influxdb]]
  database = "babylonmetricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/babylon-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_babylon.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_babylon.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_babylon.conf /etc/telegraf/telegraf_babylon.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_babylon
sudo systemctl restart telegraf_babylon
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_babylon | grep active` =~ "running" ]]; then
...
echo ''
echo -e '\e[7mYour Babylon node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_babylon -f\e[0m'
